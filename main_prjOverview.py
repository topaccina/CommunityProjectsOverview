...
totalDFIRequested = df[(df.category == 'cfp')]['dfiAmount'].sum()
totalDFIAssigned = df[(df.currentResult == 'Approved') &
                      (df.category == 'cfp')]['dfiAmount'].sum()
totalDFIAssignedPct = round(100*totalDFIAssigned/totalDFIRequested, 1)
totalCFP = df.category.value_counts().loc['cfp']
totalCFPApproved = df[(df.currentResult == 'Approved') &
                      (df.category == 'cfp')].shape[0]
totalCFPApprovedPct = round(100*totalCFPApproved/totalCFP, 1)
totalDFIP = df.category.value_counts().loc['dfip']
totalDFIPApproved = df[(df.currentResult == 'Approved')
                       & (df.category == 'dfip')].shape[0]
totalDFIPApprovedPct = round(100*totalDFIPApproved/totalDFIP, 1)
############################################################################
# category plots data
df_cfp_cat = df[(df.currentResult == 'Approved') & (
    df.category == 'cfp')]['CATEGORY'].value_counts().reset_index()
df_cfp_cat.columns = ['category', 'count']
#############################################################################
# dfi assegned/year data
df_dfi_yyyy = df[(df.currentResult == 'Approved')].groupby(df.date.astype(
    'datetime64[ns]').dt.strftime('%Y'))['dfiAmount'].sum().reset_index()
############################################################################
# dfi assegned/year/round data ---->NOT USED!!!!!!!
df_dfi_yyyy_round = df[(df.currentResult == 'Approved')].groupby([df.date.astype(
    'datetime64[ns]').dt.strftime('%Y'), 'round'])['dfiAmount'].sum().reset_index()
#############################################################################
# dfi per cfp data
df_dfi_cfp = df[(df.currentResult == 'Approved') & (
    df.category == 'cfp')][['number', 'dfiAmount']]
df_dfi_cfp_sorted = df_dfi_cfp.sort_values(by='dfiAmount', ascending=False)
##############################################################################
# masternode statistics per round
df_masternodes_round = df.groupby(
    by='date')['possible'].unique().apply(lambda x: x[0]).reset_index()
##############################################################################
# plots
...
values = df_dfi_yyyy['dfiAmount'].unique()
fig_3_1 = go.Figure(
...
fig_2_3 = go.Figure()
fig_3_2 = go.Figure(go.Scatter(x=df_masternodes_round.date, y=df_masternodes_round.possible, mode="markers+lines",
                               ...
                               df_masternodes_round['possible'].astype(str),
                               ))
...
app.layout = html.Div([
    ...
    dcc.Tabs([
                        ...
                        html.Div([
                            ...
                            html.Div([
                                ...
                                dcc.Dropdown(id='dropdown-result',
                                             options=df.currentResult.unique(),
                                             # value=df.currentResult.unique()[0],
                                             multi=False,
                                             clearable=True,
                                             className='filter-dropdown',
                                             ),
                            ...
                            dbc.Container([
                                dash_table.DataTable(
                                    ...
                                    data=df.to_dict(
                                        'records'),
                                    columns=[{'id': d, 'name': c}
                                             # for c in df.loc[:, ['title', 'dfiAmount', 'CATEGORY', 'REQUESTER', 'STATUS']]
                                             for c, d in zip(['Title', 'Result', 'DFI Amount', 'Category', 'Requester', 'Status'], df.loc[:, [
                                                 'title', 'currentResult', 'dfiAmount', 'CATEGORY', 'REQUESTER', 'STATUS']])
                                             # for c in df.columns
                                             ],
                                    #  sort_action="native",
                                 fixed_rows={
                                 ...
                                 style_data={
                                    ...
                                    style_cell_conditional=[
                                        {'if': {'column_id': ['title']},
                                         'maxWidth': '200px'},
                                        {'if': {'column_id': ['currentResult', 'dfiAmount']},
                                         'maxWidth': '50px'},
        ...
        dcc.Tab(label='Coming Soon...', value='tab3', id='tab3', className='custom-tab',
                selected_className="custom-tab--selected", children=[
                ]),
...
def CFP_DFI_bar(radio_sel):
    if radio_sel == 'All':
        x = df_dfi_cfp_sorted['number'].astype(str)
        y = df_dfi_cfp_sorted['dfiAmount']
    else:
        x = df_dfi_cfp_sorted['number'].astype(str)[0: 10]
        y = df_dfi_cfp_sorted['dfiAmount'][0: 10]

    fig_2_3 = go.Figure(go.Bar(x=x, y=y,
                               name='Assigned DFI by CFP',
                               marker=dict(color='orange'),
                               hoverinfo='text',
                               hovertext='CFP Id '+x+' assigned ' +
                               y.map(
...
def update_dropdown_options(round_v, category_v, type_v, result_v, requester_v):
    ...
    if result_v is not None:
        dff = dff[dff['currentResult'] == result_v]
...
def testTbl(active_cell, tblData):
    ...
    if selRows is not None:
        ...
        dfiAmount = df.loc[selRows]['dfiAmount']
